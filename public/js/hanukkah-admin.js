...
function getOrderInfo() {
    return {
        ...
        email: prompt("Enter parent's email address:"),
...
function uploadOrder() {
    fetch('/admin/create-hanukah-order/', {
        method: 'POST',
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify(getOrderInfo())
    }).then(async (res) => {
        if (!res.ok)
            return alert("Something went wrong: " + res.statusText);
        const { time, orderId } = await res.json();
        return alert(`Order #${orderId} successfully created at ${time}`);
